...
# Menjalankan auto report setelah executor
print_time
echo -e "${CYAN}📡 Launching auto balance report after executor...${NC}"
...
# Path ke report.js (dianggap ada di folder t3rn)
REPORT_PATH="$HOME/t3rn/report.js"
if [ ! -f "$REPORT_PATH" ]; then
  echo -e "${RED}❌ report.js not found at $REPORT_PATH. Please make sure it's there.${NC}"
  exit 1
fi

# Jalankan report.js di screen
screen -dmS ntex-report bash -c "node $REPORT_PATH <<EOF
$WALLET
$TELEGRAM_TOKEN
$CHAT_ID
$ALCHEMY_KEY
...
echo -e "${GREEN}✅ Auto report is now running in screen session 'ntex-report'.${NC}"
echo -e "🔎 Use ${CYAN}screen -r ntex-report${NC} to view the report logs."
